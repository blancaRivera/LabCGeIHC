...
void applicationLoop() {
	...
	while (psi) {
		...
		shaderColorLighting.setVectorFloat3("light.specular", glm::value_ptr(glm::vec3(0.9, 0.9, 0.0)));
		...
		glm::mat4 cubotextureModel = glm::mat4(1.0);
		cubotextureModel = glm::translate(cubotextureModel, glm::vec3(3.0, 2.0, 3.0));
		glBindTexture(GL_TEXTURE_2D, textureID4);
		shaderTexture.setVectorFloat2("scaleUV", glm::value_ptr(glm::vec2(2.0, 2.0)));
		box3.render(cubotextureModel);
		glBindTexture(GL_TEXTURE_2D, 0);
